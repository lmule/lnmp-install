...
#        handle  -lpthread -lz -lm -lrt -lcrypt -ldl -laio  and other strings inserted that might contain
...
my $socket  = '/usr/local/mysql/mysqld.sock';
my $version = '5.6.30-76.3';
...
my $flags;
$flags->{libs} =
  [@ldflags,@lib_opts,'',' -lpthread -lz -lm -lrt -ldl ','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'',' -lpthread -lz -lm -lrt -lcrypt -ldl -laio ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','',' -lpthread -lz -lm -lrt -lcrypt -ldl -laio ','',''];
